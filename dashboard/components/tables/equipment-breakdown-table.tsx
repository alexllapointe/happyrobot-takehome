...
import { equipmentBreakdown } from '@/lib/analytics';
import { fmtPct, fmtUSD } from '@/lib/format';
import type { CallRow } from '@/lib/types';

export function EquipmentBreakdownTable({ calls }: { calls: CallRow[] }) {
  const rows = equipmentBreakdown(calls);
  if (rows.length === 0) {
    return (
      <div className="py-8 text-center text-sm text-muted-foreground">
        No equipment data — calls need to be linked to a load.
      </div>
    );
  }
  return (
